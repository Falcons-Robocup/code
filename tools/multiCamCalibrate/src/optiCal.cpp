...
{
    // Hv is the homography transformation to make the forward lookup map fit nicely in the GUI
...
void opticalCalibrator::ensureForwardMaps()
{
    if (_fmapsValid == false)
    {
        calcForwardMaps();
    }
}

void opticalCalibrator::calcForwardMaps()
{
    // initialize
    cv::Mat frame = getCamFrame();
    int ncol = frame.cols;
    int nrow = frame.rows;
    _fmapx = cv::Mat(nrow, ncol, CV_16SC1, cv::Scalar(0));
    _fmapy = cv::Mat(nrow, ncol, CV_16SC1, cv::Scalar(0));
    ...
    for (size_t idx = 0; idx < pointsResult.size(); ++idx)
    {
        ...
        if (yo >= 0 && r > MIN_CUTOFF_RADIUS && r < MAX_CUTOFF_RADIUS && fabs(1e-3*xo) < MAX_CUTOFF_RADIUS && 1e-3*yo < MAX_CUTOFF_RADIUS)
        {
            ...
            {
                _fmapx.at<int16_t>(yi, xi) = (int16_t)xo;
                _fmapy.at<int16_t>(yi, xi) = (int16_t)yo;
...
cv::Mat opticalCalibrator::transformVerification(cv::Mat const &frame)
{
    ...
    for (int irow = BORDER_SIZE; irow < frame.rows - BORDER_SIZE; ++irow)
    {
        for (int icol = 0; icol < frame.cols; ++icol)
        {
            ...
            int x = _fmapx.at<int16_t>(irow, icol);
            int y = _fmapy.at<int16_t>(irow, icol);
