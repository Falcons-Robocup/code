...
void PositionBeforePOI::parseArguments()
{
    // Parse from
...
behTreeReturnEnum PositionBeforePOI::execute(bool onStarted)
{
    parseArguments();

    Vector2D targetVec = (1 - (_distance / (_to - _from).size())) * (_to - _from) + _from;
...
BT::NodeStatus PositionBeforePOI::onStart()
{
    TRACE_FUNCTION("");
...
BT::NodeStatus PositionBeforePOI::onRunning()
{
    TRACE_FUNCTION("");
...
void PositionBeforePOI::onHalted()
{
}