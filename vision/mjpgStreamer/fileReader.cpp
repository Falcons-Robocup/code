...
fileReader::fileReader(QWidget *window, QGraphicsView *camView, QGraphicsScene *camScene, QSlider *slider, mjpgSender *mjpgSend ) {
    this->window = window;
    this->slider = slider;
...
void fileReader::config() {
    ...
    if ( ! fileList.exists()) {
        ...
        QString command = "git clone https://github.com/andrepool/robocup_ml /home/robocup/robocup_ml";
        qDebug().noquote() << "          " << command;
        qDebug() <<  "WARNING this can take upto a few minutes!";

        QProcess procClone;
        procClone.start(command);
...
void fileReader::getImage() {
    ...
    // qDebug().noquote() << "INFO    index" << index << "open" << allFiles[index] << "for reading, file size" << jpegData.size() << "images" << allFiles.size();

    // imag->rotate(jpegData);
    imag->send(jpegData);
    imag->show(jpegData, allFiles[index]);
}
