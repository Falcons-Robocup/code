...
multicastReceive::multicastReceive() {
	...
	logFile = "2018-06-20_20-46-20.bin";
...
void multicastReceive::decodeStats(size_t index) {
	...
	uint8_t statsPacketSize = HEADER_SIZE + 48; // last used recvBuf.pl. below
	if( recvBuf.size != statsPacketSize ) {
		...
		uint8_t cpuTemp = recvBuf.pl.u8[32];
		uint8_t gpuTemp = recvBuf.pl.u8[33];
		uint32_t throttledValue = recvBuf.pl.u32[36 / 4];
		uint32_t frameCounter = recvBuf.pl.u32[40 / 4];
		uint32_t appUptime = recvBuf.pl.u32[44 / 4];

		printf("INFO      : CPU temp %2u GPU temp %2u throttled value 0x%08x frame counter %10d uptime %10d seconds\n",
				cpuTemp, gpuTemp, throttledValue, frameCounter, appUptime);
		...
		fprintf(writeTxt,
				"%zu %s %10s stat %3d %5d : %3d %3d %3d %3d %5d %4d : %6.1f %3.1f %3.1f : %6d %6d : %5.1f ms\n",
...
#define MAX_ROBOTS 6
		...
		else if( (strcmp(ipAddress, "172.16.74.56") == 0) || (strcmp(ipAddress, "172.16.74.122") == 0) ) {
			// the 172.16.74.122 is used the enp3s0 copper Ethernet interface
			index = 5;
		} // robot 6
