...
// usage of the LED on the camera requires to install uvcdynctrl
// after installing this package re-connect the camera to be able to use the LED control
...
#include <cerrno>
#include <dirent.h> // for DIR and structure dirent
#include <stdio.h>
#include <sys/ioctl.h>
#include <iostream>
...
cameraControl::cameraControl() {
	// be sure the configuration values are overwritten from the configuration by setting to not used value
	clearConfigCache();
...
string cameraControl::idToDescription(int id) {
	if( id == V4L2_CID_BACKLIGHT_COMPENSATION ) {
	...
	} else if( id == V4L2_CID_EXPOSURE_AUTO_PRIORITY ) {
		return "exposure auto priority";
	} else if( id == V4L2_CID_FOCUS_ABSOLUTE ) {
		return "focus absolute";
	} else if( id == V4L2_CID_FOCUS_AUTO ) {
		return "focus auto";
	} else if( id == V4L2_CID_GAIN ) {
		return "gain";
	} else if( id == V4L2_CID_LED1_FREQUENCY ) {
		return "led1 frequency";
	} else if( id == V4L2_CID_LED1_MODE ) {
		return "led1 mode";
	} else if( id == V4L2_CID_PAN_ABSOLUTE ) {
		return "pan absolute";
	} else if( id == V4L2_CID_POWER_LINE_FREQUENCY ) {
		return "power line frequency";
	} else if( id == V4L2_CID_SATURATION ) {
		return "saturation";
	} else if( id == V4L2_CID_SHARPNESS ) {
		return "sharpness";
	} else if( id == V4L2_CID_TILT_ABSOLUTE ) {
		return "tilt absolute";
	} else if( id == V4L2_CID_AUTO_WHITE_BALANCE ) {
		return "white balance auto";
	} else if( id == V4L2_CID_WHITE_BALANCE_TEMPERATURE ) {
		return "white balance";
	} else if( id == V4L2_CID_ZOOM_ABSOLUTE ) {
...
void cameraControl::update(cameraSt camera, bool verbose) {
	if( deviceIndexUsed >= 0 ) {
		...
		if( configCache.led1Frequency != camera.led1Frequency ) {
			// LED control requires library uvcdynctrl on robot
			set( V4L2_CID_LED1_FREQUENCY, camera.led1Frequency, verbose);
			configCache.led1Frequency = camera.led1Frequency;
		}

		if( configCache.led1Mode != camera.led1Mode ) {
			// LED control requires library uvcdynctrl on robot
			set( V4L2_CID_LED1_MODE, camera.led1Mode, verbose);
		...
		if( reInitAgainCounter < 20 ) {
			if( reInitAgainCounter == 19 ) {
				printf(
						"INFO      : reinitializing camera again to prevent brightness difference after camera power cycle\n");
				clearConfigCache();
			}
			reInitAgainCounter++;
		}
