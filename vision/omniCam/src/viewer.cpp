...
viewer::viewer(ballDetection *ballDet, ballPossession *ballPos, configurator *conf, ballDetection *cyanDet,
...
void viewer::roundViewerUpdate() {
...

	{
		...
		obstacleFrame.copyTo(
				selectFrame.colRange(obstacleLeft, selectFrame.cols).rowRange(obstacleTop, selectFrame.rows));
		...
		resize(possessionBgrSmall, possessionBgr, Size(), 3.0, 3.0, cv::INTER_LINEAR); // increase to zoom in on ball possession region of interest
		...
		resize(possessionInRangeFrameSmall, possessionInRangeFrame, Size(), 3.0, 3.0, cv::INTER_LINEAR); // increase to zoom in on ball possession region of interest
		...
		possessionInRangeFrame.copyTo(
				selectFrame.colRange(possessionInRangeFrameLeft, selectFrame.cols).rowRange(possessionInRangeFrameTop,
						obstacleTop));
	}
}
...
void viewer::floorPrintText() {
	...
	if( locations.size() > 0 ) {
		...
		line += 20;
		strForm.str("");
		strForm << "omniCam " << std::setprecision(1) << setw(6) << locations[0].pos.x << " " << setw(6)
...
void viewer::printText() {
	...
	if( ballView || viewMode == bgr ) {
		line += 20;
		if( balls.size() > 0 ) {
			putText(selectFrame, "Ball size angle radTan radLin radRos", Point(10, line), 1, 1, color, 1);
			line += 20;
			strForm.str("");
		} else {
			line += 20;
	...
	if( cyanView || viewMode == bgr ) {
		line += 20;
		if( cyans.size() > 0 ) {
			putText(selectFrame, "Cyan size angle radTan radLin radRos", Point(10, line), 1, 1, color, 1);
			line += 20;
			strForm.str("");
		} else {
			line += 20;
	...
	if( magentaView || viewMode == bgr ) {
		line += 20;
		if( magentas.size() > 0 ) {
			putText(selectFrame, "Magenta size angle radTan radLin radRos", Point(10, line), 1, 1, color, 1);
			line += 20;
			strForm.str("");
		} else {
			line += 20;
...
void viewer::drawCircleSections(int radius, cv::Scalar color) {
	...
	line(selectFrame, Point(centerPoint.x - 20, centerPoint.y - radius),
			Point(centerPoint.x + 20, centerPoint.y - radius), color, 1);
	line(selectFrame, Point(centerPoint.x - 20, centerPoint.y + radius),
			Point(centerPoint.x + 20, centerPoint.y + radius), color, 1);
	line(selectFrame, Point(centerPoint.x - radius, centerPoint.y - 20),
			Point(centerPoint.x - radius, centerPoint.y + 20), color, 1);
	line(selectFrame, Point(centerPoint.x + radius, centerPoint.y - 20),
			Point(centerPoint.x + radius, centerPoint.y + 20), color, 1);
}
...
void viewer::drawRaster() {
	...
	if( conf->getCalibrateMode()
			&& (viewMode == bgr || viewMode == hue || viewMode == sat || viewMode == val || viewMode == lineUnfiltered
					|| viewMode == lineErode || viewMode == lineDilate || viewMode == sunSpot || viewMode == lineSel) ) {
		// exact center pixel = 350,350 (instead of 359.5,359,5)
		// circle( selectFrame, centerPoint, conf->getCameraRadiusMin(), RASTERCOLOR, 1); // grey
		// circle( selectFrame, centerPoint, conf->getCameraRadiusMax(), RASTERCOLOR, 1);
	...
	line(selectFrame, Point(0, centerPoint.y), Point(viewPixels - 1, centerPoint.y), rasterColor, 1);
	line(selectFrame, Point(centerPoint.x, 0), Point(centerPoint.x, viewPixels - 1), rasterColor, 1);
	// draw the robot with shooter for reference
	circle(selectFrame, centerPoint, 20, blue, 1);
	if( !conf->getCalibrateMode() ) {
	...
	line(selectFrame, centerPoint, Point(centerPoint.x, centerPoint.y - 40), blue, 1);
}
...
// show the Magentas on the rectangular floor
...
void viewer::drawFloorLinePoints(positionStDbl pos, Scalar color) {
	...
	if( linePoints > conf->getLine().linePointsNumber ) {
		linePoints = conf->getLine().linePointsNumber;
	}
