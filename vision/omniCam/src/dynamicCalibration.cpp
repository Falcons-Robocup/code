...
dynamicCalibration::dynamicCalibration(configurator *conf = NULL, preprocessor *prep = NULL) {
	...
	statsFile
			<< "date,time,frame,uptime,fps,gain,whiteBal,linePixels,lineHue,lineSat,lineVal,floorPixels,floorHue,floorSat,floorVal,ballPixels,ballHue,ballSat,ballVal"
			<< endl;
}
...
int dynamicCalibration::get(string setName, int id) {
	...
	if( fd == 0 ) {
		cerr << "omniCam: dynamicCalibration Error file descriptor to camera not set" << endl;
		return 0;
	}

	if( -1 == ioctl(fd, VIDIOC_G_CTRL, &control) ) {
		cerr << std::endl;
		cerr << "omniCam: dynamicCalibration Error for getting VIDIOC_G_CTRL through ioctl for id " << setName << " (0x"
				<< hex << id << dec << ")" << endl;
...
void dynamicCalibration::set(string setName, int id, int value) {
	...
	if( -1 == ioctl(fd, VIDIOC_S_CTRL, &control) ) {
		cerr << endl;
		cerr << "omniCam: dynamicCalibration Error for setting VIDIOC_S_CTRL through ioctl for id " << setName << " (0x"
				<< hex << id << dec << ") with value " << value << endl;
		cerr << "Error: " << strerror(errno) << endl;
		cerr
				<< "The camera control only works for newer Logitech c525 camera's the early c525 lack a number of essential controls"
				<< endl;
		exit(EXIT_FAILURE);
	}
}
...
void dynamicCalibration::nameSet(string setName, int id, int prevVal, int newVal, bool verbose) {
	...
	if( id == V4L2_CID_WHITE_BALANCE_TEMPERATURE ) {
		if( get("white balance auto", V4L2_CID_AUTO_WHITE_BALANCE) == 1 ) {
			// cerr << "omniCam: dynamicCalibration Error not able to set white balance because auto white balance is enabled" << endl;
			return;
		}
	}
	...
	if( prevVal != newVal ) {
		...
		if( newVal != readBack ) {
			cout << "omniCam: dynamicCalibration error: " << setName << " is " << setw(4) << readBack
					<< " but should be " << setw(4) << newVal << endl;
		}
	}
}
...
void dynamicCalibration::initialize(int fd) {
	...
	if( -1 == ioctl(fd, VIDIOC_QUERYCAP, &caps) ) {
		cerr << endl;
		cerr << "omniCam: dynamicCalibration Error when querying capabilities" << endl;
		cerr << "Error: " << strerror(errno) << endl;
		exit(EXIT_FAILURE);
	}
...
void dynamicCalibration::update() {
	...
	nameSet("white balance", V4L2_CID_WHITE_BALANCE_TEMPERATURE, cameraPrev.whiteBalanceTemperature,
			camera.whiteBalanceTemperature, verbose);
...
void dynamicCalibration::captureStats(uint64 frame) {
	...
	double fps = fieldTimeSize * 1.0
			/ ((timeData.fieldTime[0] - timeData.fieldTime[fieldTimeSize - 1]) / getTickFrequency());
