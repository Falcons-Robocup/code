...
// round viewer, which is the cropped de-warped image from the camera.
// The floor is generated in the class constructor, so each time the omniCam is started.
// The floor is created from the dimensions defined in the omniCam.yaml file.
...
// In omniCam all sizes are defined from the center of the line.
// The MSL specification is in meters.
// In omniCam all sizes are in pixels.
...
robotFloor::robotFloor(configurator *conf) {
	...
	// Inside omniCam all dimensions are in pixels and at the center of the lines.
	...
	if( goalDepth < 0 ) {
		goalDepth = 0;
	} // prevent a negative goalDepth in case the goal post "line" is not needed (and set to 0)
	...
	if( A % 2 == 1 ) {
		A--;
		cout << "A ";
	} // APOX TODO: investigate if error is less with -- or ++
	if( B % 2 == 1 ) {
		B--;
		cout << "B ";
	}
	...
	cout << endl;
	cout << "robotFloor: A " << A << " B " << B << " C " << C << " D " << D << " E " << E << " F " << F << " G " << G
			<< " H " << H << " I " << I << " J " << J << " K " << K << endl;
	cout << "robotFloor: goal line border " << goalLineBorder << " touch line border " << touchLineBorder
			<< " line thickness " << lineThickness << " meters to pixels " << metersToPixels << endl;

	floorSizeFile.open("FloorFileSize1.yaml");
	floorSizeFile << "%YAML:1.0" << endl;
	floorSizeFile << "# This file is created/overwritten each time omniCam is started." << endl;
	floorSizeFile << "# It is mainly for debugging." << endl;
	floorSizeFile << "# The information is calculated from: omniCam.yaml:dimensions" << endl;
	...
	goalSize.xDepth = goalDepth;
	; // to prevent wrong locking, APOX todo: remove
...
void robotFloor::createNormal() {
	...
	if( goalPostOut > 0 ) {
		line(frameCost, leftTopGoalBlockRight + Point(-3, -goalPostOut),
				leftTopGoalBlockRight + Point(-30, -10 * goalPostOut), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, leftBottomGoalBlockRight + Point(-3, goalPostOut),
				leftBottomGoalBlockRight + Point(-30, 10 * goalPostOut), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, rightTopGoalBlockLeft + Point(3, -goalPostOut),
				rightTopGoalBlockLeft + Point(30, -10 * goalPostOut), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, rightBottomGoalBlockLeft + Point(3, goalPostOut),
				rightBottomGoalBlockLeft + Point(30, 10 * goalPostOut), lineColor, drawLineThickness); // block left top goal post
	}

	int goalPostIn = conf->getGoalPostIn();
	if( goalPostIn > 0 ) {
		line(frameCost, leftTopGoalBlockRight + Point(4, -goalPostIn),
				leftTopGoalBlockRight + Point(40, -10 * goalPostIn), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, leftBottomGoalBlockRight + Point(4, goalPostIn),
				leftBottomGoalBlockRight + Point(40, 10 * goalPostIn), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, rightTopGoalBlockLeft + Point(-4, -goalPostIn),
				rightTopGoalBlockLeft + Point(-40, -10 * goalPostIn), lineColor, drawLineThickness); // block left top goal post
		line(frameCost, rightBottomGoalBlockLeft + Point(-4, goalPostIn),
				rightBottomGoalBlockLeft + Point(-40, 10 * goalPostIn), lineColor, drawLineThickness); // block left top goal post
	}
	...
	Mat cornerArc = Mat::zeros(cornerSize.arcRadius + cornerSize.arcLineThickness / 2,
			cornerSize.arcRadius + cornerSize.arcLineThickness / 2, CV_8UC1);
	...
	cornerArc.copyTo(
			frameCost.colRange(fieldSize.xLeft, fieldSize.xLeft + cornerArc.cols).rowRange(fieldSize.yTop,
					fieldSize.yTop + cornerArc.rows));
	cv::flip(cornerArc, cornerArc, 1);
	cornerArc.copyTo(
			frameCost.colRange(fieldSize.xRight - cornerArc.cols + 1, fieldSize.xRight + 1).rowRange(fieldSize.yTop,
					fieldSize.yTop + cornerArc.rows));
	cv::flip(cornerArc, cornerArc, 0);
	cornerArc.copyTo(
			frameCost.colRange(fieldSize.xRight - cornerArc.cols + 1, fieldSize.xRight + 1).rowRange(
					fieldSize.yBottom - cornerArc.rows + 1, fieldSize.yBottom + 1));
	cv::flip(cornerArc, cornerArc, 1);
	cornerArc.copyTo(
			frameCost.colRange(fieldSize.xLeft, fieldSize.xLeft + cornerArc.cols).rowRange(
					fieldSize.yBottom - cornerArc.rows + 1, fieldSize.yBottom + 1));
...
void robotFloor::blurPixel(float blurValue) {
	...
	for( int x = 1; x < floorSize.xRight; x++ ) { // range is from 0 to <= xFloorRight, but use difference for xDelta
		for( int y = 1; y < floorSize.yBottom; y++ ) { // range is from 0 to <= yFloorBottom, but use difference for yDelta
			...
			if( outputFloor.at<uchar>(y, x) < pixelVal ) {
				outputFloor.at<uchar>(y, x) = pixelVal;
			} // center pixel
			pixelVal = (int)(blurValue * pixelVal + 0.5f);
			if( outputFloor.at<uchar>(y - 1, x - 1) < pixelVal ) {
				outputFloor.at<uchar>(y - 1, x - 1) = pixelVal;
			} // bottom left
			if( outputFloor.at<uchar>(y - 1, x) < pixelVal ) {
				outputFloor.at<uchar>(y - 1, x) = pixelVal;
			} // bottom
			if( outputFloor.at<uchar>(y - 1, x + 1) < pixelVal ) {
				outputFloor.at<uchar>(y - 1, x + 1) = pixelVal;
			} // bottom right
			if( outputFloor.at<uchar>(y, x + 1) < pixelVal ) {
				outputFloor.at<uchar>(y, x + 1) = pixelVal;
			} // right
			if( outputFloor.at<uchar>(y + 1, x + 1) < pixelVal ) {
				outputFloor.at<uchar>(y + 1, x + 1) = pixelVal;
			} // top right
...
void robotFloor::createCalibrated() {
	...
	// NOTE: MSL specification measures outside of line (because line is part of the 1200x1800cm field)
	// omniCam uses center of lines, so for omniCam the field is a little smaller
...
bool robotFloor::inField(Point position) {
	return (position.x >= fieldSize.xLeft && position.x <= fieldSize.xRight && position.y >= fieldSize.yTop
			&& position.y <= fieldSize.yBottom);
}
