...
determinePosition::determinePosition(configurator *conf, linePointDetection *linePoint, preprocessor *prep,
	...
	for( unsigned int ii = 0; ii < numThreads; ii++ ) {
		...
		threads[ii].id = ii;
		threads[ii].classContext = this; // be able to access this class from the threads
...
detPosSt determinePosition::optimizePosition(positionStDbl startPos, bool localSearch,
	...
	if( result.pos.x < 0.0f || result.pos.x > (double)rFloor->getXFloorRight() ) {
		printf("WARNING   : solver x %f out of range", result.pos.x); // should never happen
	}
	if( result.pos.y < 0.0f || result.pos.y > (double)rFloor->getYFloorBottom() ) {
		printf("WARNING   : solver y %f out of range", result.pos.y); // should never happen
	}
	if( result.pos.rz >= rzMin && result.pos.rz < 0 ) {
		...
		printf("WARNING   : solver rz %f out of range", result.pos.rz); // should never happen
	}
	return result;
}

// function that is run from the different localization threads
void* determinePosition::processOneLocation(void *id) {
	...
	if( context->found.score < context->scoreThreshHold ) {
		...
		if( context->found.score != tmp.score ) {
			printf("WARNING   : recalculation score is %.3f but calculation best position score is %.3f",
					context->found.score, tmp.score);
		}
	}
	return 0;
}
...
// if we discard close points we might end up in a situation that a wrong found location
...
// that was already in the search list. That will cost one additional search, but in a few
// iterations the two will be merged again to one.
// So better to do the double search than have a blocking mechanism.
positionStDbl determinePosition::createRandomPosition() {
	positionStDbl pos;
...
void determinePosition::pointsToPosition() {
	if( conf->getManualMode() ) {
		...
		detPosSt randomLoc;
		randomLoc.pos = createRandomPosition();
		randomLoc.score = 1.0f; // make invalid, the solver will update the score, if it stays to low, it will later be thrown out again
		randomLoc.lastActive = 0;
		randomLoc.age = 0;
		locList.push_back(randomLoc);

		// now run localization algorithm on all previous found positions, including the new random location
		...
		if( locSize > numThreads ) {
			printf("ERROR     : not enough threads available to run all localization searches");
			exit(EXIT_FAILURE);
		}
		...
		for( uint ii = 0; ii < locSize; ii++ ) {
			...
			if( pthread_create(&(threads[ii].thread), NULL, processOneLocation, &(threads[ii])) ) {
				printf("ERROR     : cannot create localization thread %u", ii);
				exit(EXIT_FAILURE);
			}
		}
