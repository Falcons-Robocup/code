...
configurator::configurator(int robot) {
	...
	if( robot == 1 ) {
		...
		std::cerr << "omniCam: Robot " << robot << " has no configuration" << std::endl;
		exit(1);
	}
...
void configurator::init(int floorWidth, int floorHeight, bool guiEnabled = false) {
	...
	if( floorWidth % 2 == 0 ) {
		cerr << "omniCam: configurator Error: floor width should be an odd number but is: " << floorWidth << endl;
		;
		exit(1);
	}
	if( floorHeight % 2 == 0 ) {
		cerr << "omniCam: configurator Error: floor height should be an odd number but is: " << floorHeight << endl;
		;
		exit(1);
	}
...
void configurator::showTrackbars(int floorWidth, int floorHeight) {
	...
	namedWindow(colorWindowFloor, cv::WINDOW_NORMAL);
	createTrackbar("f Hue Cent", colorWindowFloor, &floorSt.hue.center, 179, &configurator::cbUpdateFloorTrackbar,
			this); // hue range is 0 to 179
	createTrackbar("f Hue Delta", colorWindowFloor, &floorSt.hue.delta, 180, &configurator::cbUpdateFloorTrackbar,
			this);
	...
	namedWindow(colorWindowPossession, cv::WINDOW_NORMAL);
	createTrackbar("p Hue Cent", colorWindowPossession, &possession.hue.center, 179,
			&configurator::cbUpdatePossessionTrackbar, this); // hue range is 0 to 179
	createTrackbar("p Hue Delta", colorWindowPossession, &possession.hue.delta, 180,
			&configurator::cbUpdatePossessionTrackbar, this);
	createTrackbar("p Sat Min", colorWindowPossession, &possession.sat.min, 255,
			&configurator::cbUpdatePossessionTrackbar, this);
	createTrackbar("p Sat Max", colorWindowPossession, &possession.sat.max, 255,
			&configurator::cbUpdatePossessionTrackbar, this);
	createTrackbar("p Val Min", colorWindowPossession, &possession.val.min, 255,
			&configurator::cbUpdatePossessionTrackbar, this);
	createTrackbar("p Val Max", colorWindowPossession, &possession.val.max, 255,
			&configurator::cbUpdatePossessionTrackbar, this);
	...
	createTrackbar("o Hue Cent", colorWindowObstacle, &obstacle.hue.center, 179,
			&configurator::cbUpdateObstacleTrackbar, this); // hue range is 0 to 179
	createTrackbar("o Hue Delta", colorWindowObstacle, &obstacle.hue.delta, 180,
			&configurator::cbUpdateObstacleTrackbar, this);
	createTrackbar("o Sat Min", colorWindowObstacle, &obstacle.sat.min, 255, &configurator::cbUpdateObstacleTrackbar,
			this);
	createTrackbar("o Sat Max", colorWindowObstacle, &obstacle.sat.max, 255, &configurator::cbUpdateObstacleTrackbar,
			this);
	createTrackbar("o Val Min", colorWindowObstacle, &obstacle.val.min, 255, &configurator::cbUpdateObstacleTrackbar,
			this);
	createTrackbar("o Val Max", colorWindowObstacle, &obstacle.val.max, 255, &configurator::cbUpdateObstacleTrackbar,
			this);
	...
	namedWindow(colorWindowCyan, cv::WINDOW_NORMAL);
	...
	createTrackbar("c Dilate", colorWindowCyan, &cyan.dilateSlider, 20, &configurator::cbUpdateCyanDilateTrackbar,
			this);
	...
	namedWindow(colorWindowMagenta, cv::WINDOW_NORMAL);
	createTrackbar("m Hue Cent", colorWindowMagenta, &magenta.hue.center, 179, &configurator::cbUpdateMagentaTrackbar,
			this); // hue range is 0 to 179
	createTrackbar("m Hue Delta", colorWindowMagenta, &magenta.hue.delta, 180, &configurator::cbUpdateMagentaTrackbar,
			this);
	createTrackbar("m Sat Min", colorWindowMagenta, &magenta.sat.min, 255, &configurator::cbUpdateMagentaTrackbar,
			this);
	createTrackbar("m Sat Max", colorWindowMagenta, &magenta.sat.max, 255, &configurator::cbUpdateMagentaTrackbar,
			this);
	createTrackbar("m Val Min", colorWindowMagenta, &magenta.val.min, 255, &configurator::cbUpdateMagentaTrackbar,
			this);
	createTrackbar("m Val Max", colorWindowMagenta, &magenta.val.max, 255, &configurator::cbUpdateMagentaTrackbar,
			this);
	createTrackbar("m Erode", colorWindowMagenta, &magenta.erodeSlider, 20, &configurator::cbUpdateMagentaErodeTrackbar,
			this);
	createTrackbar("m Dilate", colorWindowMagenta, &magenta.dilateSlider, 20,
			&configurator::cbUpdateMagentaDilateTrackbar, this);
	...
	cameraWindow = "camera";
	namedWindow(cameraWindow, cv::WINDOW_NORMAL);
...
void configurator::updateLineTrackbar(int) {
	if( lineSt.sat.max < lineSt.sat.min ) {
		lineSt.sat.max = lineSt.sat.min;
		setTrackbarPos("l Sat Max", colorWindowLine, lineSt.sat.max);
	}
	if( lineSt.sat.min > lineSt.sat.max ) {
		lineSt.sat.min = lineSt.sat.max;
		setTrackbarPos("l Sat Min", colorWindowLine, lineSt.sat.min);
	}
	if( lineSt.val.max < lineSt.val.min ) {
		lineSt.val.max = lineSt.val.min;
		setTrackbarPos("l Val Max", colorWindowLine, lineSt.val.max);
	}
	if( lineSt.val.min > lineSt.val.max ) {
		lineSt.val.min = lineSt.val.max;
		setTrackbarPos("l Val Min", colorWindowLine, lineSt.val.min);
	}
	drawLineBGR();
}
void configurator::updateFloorTrackbar(int) {
	if( floorSt.sat.max < floorSt.sat.min ) {
		floorSt.sat.max = floorSt.sat.min;
		setTrackbarPos("f Sat Max", colorWindowFloor, floorSt.sat.max);
	}
	if( floorSt.sat.min > floorSt.sat.max ) {
		floorSt.sat.min = floorSt.sat.max;
		setTrackbarPos("f Sat Min", colorWindowFloor, floorSt.sat.min);
	}
	if( floorSt.val.max < floorSt.val.min ) {
		floorSt.val.max = floorSt.val.min;
		setTrackbarPos("f Val Max", colorWindowFloor, floorSt.val.max);
	}
	if( floorSt.val.min > floorSt.val.max ) {
		floorSt.val.min = floorSt.val.max;
		setTrackbarPos("f Val Min", colorWindowFloor, floorSt.val.min);
	}
	drawFloorBGR();
}
void configurator::updateBallTrackbar(int) {
	if( ball.sat.max < ball.sat.min ) {
		ball.sat.max = ball.sat.min;
		setTrackbarPos("b Sat Max", colorWindowBall, ball.sat.max);
	}
	if( ball.sat.min > ball.sat.max ) {
		ball.sat.min = ball.sat.max;
		setTrackbarPos("b Sat Min", colorWindowBall, ball.sat.min);
	}
	if( ball.val.max < ball.val.min ) {
		ball.val.max = ball.val.min;
		setTrackbarPos("b Val Max", colorWindowBall, ball.val.max);
	}
	if( ball.val.min > ball.val.max ) {
		ball.val.min = ball.val.max;
		setTrackbarPos("b Val Min", colorWindowBall, ball.val.min);
	}
	drawBallBGR();
}
void configurator::updatePossessionTrackbar(int) {
	if( possession.sat.max < possession.sat.min ) {
		possession.sat.max = possession.sat.min;
		setTrackbarPos("p Sat Max", colorWindowPossession, possession.sat.max);
	}
	if( possession.sat.min > possession.sat.max ) {
		possession.sat.min = possession.sat.max;
		setTrackbarPos("p Sat Min", colorWindowPossession, possession.sat.min);
	}
	if( possession.val.max < possession.val.min ) {
		possession.val.max = possession.val.min;
		setTrackbarPos("p Val Max", colorWindowPossession, possession.val.max);
	}
	if( possession.val.min > possession.val.max ) {
		possession.val.min = possession.val.max;
		setTrackbarPos("p Val Min", colorWindowPossession, possession.val.min);
	}
	possession.hue.min = possession.hue.center - (possession.hue.delta - 1) / 2; // -1 for slider granularity
	possession.hue.max = possession.hue.center + possession.hue.delta / 2;
}
void configurator::updateObstacleTrackbar(int) {
	if( obstacle.sat.max < obstacle.sat.min ) {
		obstacle.sat.max = obstacle.sat.min;
		setTrackbarPos("o Sat Max", colorWindowObstacle, obstacle.sat.max);
	}
	if( obstacle.sat.min > obstacle.sat.max ) {
		obstacle.sat.min = obstacle.sat.max;
		setTrackbarPos("o Sat Min", colorWindowObstacle, obstacle.sat.min);
	}
	if( obstacle.val.max < obstacle.val.min ) {
		obstacle.val.max = obstacle.val.min;
		setTrackbarPos("o Val Max", colorWindowObstacle, obstacle.val.max);
	}
	if( obstacle.val.min > obstacle.val.max ) {
		obstacle.val.min = obstacle.val.max;
		setTrackbarPos("o Val Min", colorWindowObstacle, obstacle.val.min);
	}
	drawObstacleBGR();
}
void configurator::updateCyanTrackbar(int) {
	if( cyan.sat.max < cyan.sat.min ) {
		cyan.sat.max = cyan.sat.min;
		setTrackbarPos("c Sat Max", colorWindowCyan, cyan.sat.max);
	}
	if( cyan.sat.min > cyan.sat.max ) {
		cyan.sat.min = cyan.sat.max;
		setTrackbarPos("c Sat Min", colorWindowCyan, cyan.sat.min);
	}
	if( cyan.val.max < cyan.val.min ) {
		cyan.val.max = cyan.val.min;
		setTrackbarPos("c Val Max", colorWindowCyan, cyan.val.max);
	}
	if( cyan.val.min > cyan.val.max ) {
		cyan.val.min = cyan.val.max;
		setTrackbarPos("c Val Min", colorWindowCyan, cyan.val.min);
	}
	drawCyanBGR();
}
void configurator::updateMagentaTrackbar(int) {
	if( magenta.sat.max < magenta.sat.min ) {
		magenta.sat.max = magenta.sat.min;
		setTrackbarPos("m Sat Max", colorWindowMagenta, magenta.sat.max);
	}
	if( magenta.sat.min > magenta.sat.max ) {
		magenta.sat.min = magenta.sat.max;
		setTrackbarPos("m Sat Min", colorWindowMagenta, magenta.sat.min);
	}
	if( magenta.val.max < magenta.val.min ) {
		magenta.val.max = magenta.val.min;
		setTrackbarPos("m Val Max", colorWindowMagenta, magenta.val.max);
	}
	if( magenta.val.min > magenta.val.max ) {
		magenta.val.min = magenta.val.max;
		setTrackbarPos("m Val Min", colorWindowMagenta, magenta.val.min);
	}
	drawMagentaBGR();
}
...
void configurator::drawLineBGR() {
	...
	for( int col = 0; col < lineStHSV.cols; col++ ) {
		...
		for( int row = 0; row < lineStHSV.rows; row++ ) {
			int sat = lineSt.sat.min
					+ (int)(0.5f + (1.f * row * (lineSt.sat.max - lineSt.sat.min)) / (lineStHSV.rows - 1)); // Saturation
...
void configurator::drawFloorBGR() {
	...
	for( int col = 0; col < floorStHSV.cols; col++ ) {
		int hue = floorSt.hue.min
				+ (int)(0.5f + (1.f * col * (floorSt.hue.max - floorSt.hue.min)) / (floorStHSV.cols - 1));
		if( hue < 0 ) {
			hue = hue + 180;
		}
		if( hue > 179 ) {
		...
		for( int row = 0; row < floorStHSV.rows; row++ ) {
			int sat = floorSt.sat.min
					+ (int)(0.5f + (1.f * row * (floorSt.sat.max - floorSt.sat.min)) / (floorStHSV.rows - 1)); // Saturation
...
void configurator::drawObstacleBGR() {
	...
	for( int col = 0; col < obstacleHSV.cols; col++ ) {
		int hue = obstacle.hue.min
				+ (int)(0.5f + (1.f * col * (obstacle.hue.max - obstacle.hue.min)) / (obstacleHSV.cols - 1));
		if( hue < 0 ) {
			hue = hue + 180;
		}
		if( hue > 179 ) {
		...
		for( int row = 0; row < obstacleHSV.rows; row++ ) {
			int sat = obstacle.sat.min
					+ (int)(0.5f + (1.f * row * (obstacle.sat.max - obstacle.sat.min)) / (obstacleHSV.rows - 1)); // Saturation
...
void configurator::drawMagentaBGR() {
	...
	for( int col = 0; col < magentaHSV.cols; col++ ) {
		int hue = magenta.hue.min
				+ (int)(0.5f + (1.f * col * (magenta.hue.max - magenta.hue.min)) / (magentaHSV.cols - 1));
		if( hue < 0 ) {
			hue = hue + 180;
		}
		if( hue > 179 ) {
		...
		for( int row = 0; row < magentaHSV.rows; row++ ) {
			int sat = magenta.sat.min
					+ (int)(0.5f + (1.f * row * (magenta.sat.max - magenta.sat.min)) / (magentaHSV.rows - 1)); // Saturation
...
double configurator::calcDeWarp(double pixel) {
	return deWarp.outSideTan * tan(pixel * deWarp.inSideTan) + pixel * deWarp.pixelPow1
			+ pow(pixel, 2) * deWarp.pixelPow2;
}
