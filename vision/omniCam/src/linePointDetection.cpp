...
//
// TODO: update information that it are no longer spiral lines but straight lines on 3 different angles
...
void linePointDetection::update() {
	...
	for( size_t jj = 0; jj < spiralLines.size(); jj++ ) {
		...
		for( size_t ii = 0; ii < spiralLines[jj].size(); ii++ ) {
			...
			if( onTheLine ) {
				linePointsAll.push_back(spiralLines[jj].at(ii));
			} // show all found white pixels on grid lines as small grey lines in the round viewer
...
void linePointDetection::detectSunSpot() {
	...
	findContours(contourFrame, sunSpotContours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE); // retrieves external contours
	...
	for( size_t ii = 0; ii < sunSpotContours.size(); ii++ ) {
		...
		if( width > conf->getLine().sunSpotWidth && height > conf->getLine().sunSpotHeight
				&& sunBoxPixels > requiredPixels ) {
			// cout << "white: ii " << ii << " width " << width << " height " << height << " size " << sunBoxPixels << " required " << requiredPixels << endl;
			drawContours(sunSpotFrame, sunSpotContours, ii, 255, cv::FILLED, 8, hierarchy, 0, Point());
